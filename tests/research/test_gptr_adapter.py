...
def test_gptr_config_requires_project_for_vertex_backend(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("GOOGLE_CLOUD_PROJECT", raising=False)
    monkeypatch.delenv("GCP_PROJECT", raising=False)

    assert (
        _gptr_config_unavailable(_settings(google_cloud_project=None))
        == "missing_google_cloud_project"
    )


def test_gptr_env_for_vertex_backend_sets_google_genai_vertex_mode() -> None:
    ...
    assert env["GOOGLE_CLOUD_PROJECT"] == "counselle-dev"
    assert env["GOOGLE_CLOUD_LOCATION"] == "us-central1"
    assert env["GOOGLE_API_KEY"] == "vertex-key"
...
@pytest.mark.asyncio
async def test_gptr_missing_project_fails_fast_before_import(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.delenv("GOOGLE_CLOUD_PROJECT", raising=False)
    monkeypatch.delenv("GCP_PROJECT", raising=False)
    ...
    assert result == {
        "results": [],
        "cost_usd": None,
        "unavailable": "missing_google_cloud_project",
    }
