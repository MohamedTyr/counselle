...
function hasValidReference(
  data: unknown,
): data is NonNullable<ReturnType<typeof useApplication>["data"]> {
      ...
      <section className="flex min-h-0 flex-1 items-start p-6">
        <div className="flex max-w-md flex-col gap-3 rounded-xl border bg-card p-6">
          <h1 className="font-heading text-lg font-medium">
            Could not load school workspace
          </h1>
          <p className="text-sm text-muted-foreground">
            The school details or reference catalog could not be loaded. This is
            not shown as an empty catalog because that would hide a data
            failure.
          </p>
          <div className="flex flex-wrap gap-2">
            <Button onClick={() => void detail.refetch()}>Try again</Button>
            <Button
              onClick={() => void navigate("/app/schools")}
              variant="outline"
            >
              Back to schools
            </Button>
