...
import { adaptLegacyCompletedTurn, adaptStoredTranscript, isLegacySourceEntry } from "@/api/chat/legacy-replay";
...
describe("shared protocol fixtures", () => {
  test("stored malformed known-v2 visualizations become safe opaque records", () => {
    const [entry] = adaptStoredTranscript([{
      ...
      parts: [{
        type: "viz",
        spec: { v: 2, type: "comparison_table", title: "Unsafe table", columns: [], rows: [], secret: "drop me" },
      }],
      status: "complete",
    }]);
    if (entry?.role !== "assistant" || entry.parts?.[0]?.type !== "viz") throw new Error("stored fixture was not adapted");
    expect(entry.parts[0].spec).toEqual({ v: 2, type: "comparison_table", title: "Unsafe table" });
  ...
  it("replays the separate legacy v1 fixture without admitting it to live SSE", async () => {
    ...
    expect(legacy?.sources?.[0] && isLegacySourceEntry(legacy.sources[0])).toBe(true);
    expect(legacy?.sources?.[0]?.citation.source).toBe("ipeds");

    const legacyEvent = frameFromLegacySource(fixture.turn_records[0]);
    await expect(parseFixture(JSON.stringify({ events: [legacyEvent] }))).rejects.toThrow("malformed sources");

    const current = JSON.parse(turnFullRaw) as { events: ProtocolEvent[] };
    await expect(parseFixture(turnFullRaw)).resolves.toHaveLength(current.events.length);
  ...
  it("pins the current golden to the v1 event/v2 evidence contract", () => {
    ...
    const viz = payload.events.find(
      (event): event is Extract<ProtocolEvent, { type: "viz" }> => event.type === "viz",
    );
    ...
    if (!viz || !isTabularRenderSpec(viz.data)) throw new Error("current fixture lacks a tabular viz");
    const cells = viz.data.rows.flatMap((row) => row.cells);
    expect(cells).toEqual(expect.arrayContaining([
      expect.objectContaining({
        v: 2,
        available: true,
        citation: expect.objectContaining({ source: "cds", document_sha256: expect.any(String) }),
        evidence: expect.objectContaining({ eid: expect.any(String), page: expect.any(Number) }),
        caveats: expect.arrayContaining([expect.objectContaining({ kind: expect.any(String), text: expect.any(String) })]),
      }),
      expect.objectContaining({ available: true, citation: expect.objectContaining({ source: "web" }) }),
      expect.objectContaining({ available: false, citation: null, evidence: null, marker: null }),
    ]));

    const sources = payload.events.find(
      (event): event is Extract<ProtocolEvent, { type: "sources" }> => event.type === "sources",
    );
    expect(sources?.data.sources).toEqual(expect.arrayContaining([
      expect.objectContaining({
        v: 2,
        citation: expect.objectContaining({ source: "cds" }),
        evidence: expect.arrayContaining([expect.objectContaining({ eid: expect.any(String) })]),
      }),
      expect.objectContaining({ citation: expect.objectContaining({ source: "web" }), evidence: [] }),
    ]));

    const webStep = payload.events.find(
      (event): event is Extract<ProtocolEvent, { type: "step" }> =>
        event.type === "step" && event.data.kind === "web_search" && event.data.status === "end",
    );
    ...
    const dbStep = payload.events.find(
      (event): event is Extract<ProtocolEvent, { type: "step" }> =>
        event.type === "step" && event.data.kind === "db_tool" && event.data.status === "end",
    );
    expect(dbStep?.data.detail).toMatchObject({ tool: "get_domain", domain_id: "admissions" });
...
function frameFromLegacySource(value: unknown): ProtocolEvent {
  ...
  return { v: 1, type: "sources", data: { sources: turn.sources } } as ProtocolEvent;
}
