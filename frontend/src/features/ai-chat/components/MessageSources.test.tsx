...
function source(index: number, citation: Citation): SourceEntry {
  return {
    v: 2,
    index,
    label:
      citation.source === "cds"
        ? "North College — Common Data Set 2024-25"
        : "External",
    citation,
    evidence:
      citation.source === "cds"
...
const spec: RenderSpec = {
  ...
  type: "stat_block",
  title: "Rate",
  columns: [{ unitid: 1, name: "North College" }],
  rows: [
    {
      label: "Rate",
      cells: [
...
describe("MessageSources", () => {
  test("renders actual prose and viz documents, including viz-only CDS, with no synthetic credit", () => {
    ...
    render(
      <MessageSources
        message={{
          ...
          text: "",
          sources: [source(1, web), source(2, cds), source(9, cds)],
          turnStatus: "complete",
        }}
        onOpen={onOpen}
      />,
    ...
    expect(JSON.stringify(onOpen.mock.calls[0][0])).not.toContain(
      "counselle-data",
    );
  ...
  test("renders nothing mid-stream or for uncited cumulative rows", () => {
    const { container, rerender } = render(
      <MessageSources
        message={{
          blocks: [{ kind: "markdown", text: "No marker." }],
          text: "",
          sources: [source(2, cds)],
