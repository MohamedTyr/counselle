...
function prompt(
  overrides: Partial<SchoolEssayPrompt> = {},
): SchoolEssayPrompt {
  return {
...
describe("SchoolWorkspace honesty states", () => {
  it("renders a loaded empty catalog as unavailable data without inventing facts", async () => {
    renderApp(`/app/schools/${workspaceApplicationFixture.id}`, {
      fetchHandler: (input, init) => {
        if (String(input).includes(`/v1/applications/${workspaceApplicationFixture.id}`)) {
          return jsonResponse({
    ...
    expect(
      await screen.findByText("No catalog data for the 2026-27 application cycle"),
    ).toBeInTheDocument()
    expect(
      screen.getByText(/Nothing is inferred or fabricated\./),
    ).toBeInTheDocument()
    expect(screen.getByText("Common items to verify")).toBeInTheDocument()
    expect(screen.queryByText("Published school requirements")).not.toBeInTheDocument()
  })
  ...
  it("renders a query failure as an error instead of an empty catalog", async () => {
    renderApp(`/app/schools/${workspaceApplicationFixture.id}`, {
      fetchHandler: (input, init) => {
        if (String(input).includes(`/v1/applications/${workspaceApplicationFixture.id}`)) {
          return jsonResponse({ detail: "catalog unavailable" }, { status: 500 })
        }
        return defaultAuthenticatedFetch(input, init)
      },
    })
    ...
    expect(
      await screen.findByRole("heading", { name: "Could not load school workspace" }),
    ).toBeInTheDocument()
    expect(
  ...
  it("suppresses draft and tracking actions for published not-required items", async () => {
    ...
    expect(screen.queryByLabelText("Tracking status for Application fee")).not.toBeInTheDocument()
  ...
  it("archives from the workspace and offers an undo restore", async () => {
    ...
    renderApp(`/app/schools/${workspaceApplicationFixture.id}`, { fetchHandler })
  ...
  it("reloads the cycle-scoped reference after changing cycle year", async () => {
    ...
    expect(
      await screen.findByText("No catalog data for the 2027-28 application cycle"),
    ).toBeInTheDocument()
  })
})
