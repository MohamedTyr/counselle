...
function serializedDatetime(value: unknown): boolean {
  ...
  return Number(year) > 0 && Number(month) >= 1 && Number(month) <= 12 && Number(day) >= 1 &&
    Number(day) <= daysInMonth && Number(hour) <= 23 && Number(minute) <= 59 &&
...
export function isCurrentCitation(value: unknown): value is Citation {
  ...
  if (value.v !== 2 || typeof value.source !== "string" || !sourceNames.has(value.source as SourceName) ||
      ...
      (!("retrieved_at" in value) || nullableString(value.retrieved_at)) === false ||
...
export function isCurrentEvidence(value: unknown): value is EvidenceItem {
  return record(value) && only(value, ["eid", "value_display", "label", "page", "section", "row_label", "column_label", "excerpt"]) &&
    nonEmpty(value.eid) && typeof value.value_display === "string" && nonEmpty(value.label) &&
    positiveInteger(value.page) && (!("section" in value) || nullableString(value.section)) &&
    (!("row_label" in value) || nullableString(value.row_label)) &&
    (!("column_label" in value) || nullableString(value.column_label)) && nonEmpty(value.excerpt);
}
...
function envelope(value: unknown): value is CitationEnvelope {
  if (!record(value) || !only(value, ["v", "field", "label", "display", "raw", "available", "unit", "citation", "evidence", "caveats", "marker"]) ||
      value.v !== 2 || !(value.field === null || typeof value.field === "string") || typeof value.label !== "string" ||
      typeof value.display !== "string" || typeof value.available !== "boolean" ||
      (!("raw" in value) || jsonValue(value.raw)) === false || (!("unit" in value) || nullableString(value.unit)) === false ||
      !Array.isArray(value.caveats) || !value.caveats.every((item) => record(item) && only(item, ["kind", "text"]) && typeof item.kind === "string" && /^[a-z][a-z0-9_]*$/.test(item.kind) && nonEmpty(item.text))) return false;
  if (!value.available) return value.display === "not available" && ["raw", "unit", "citation", "evidence", "marker"].every((key) => absentOrNull(value, key));
  if (!nonEmpty(value.display) || !isCurrentCitation(value.citation) || !nonEmpty(value.marker) || !/^\[[1-9]\d*\]$/.test(value.marker)) return false;
  if (value.citation.source !== "cds") return absentOrNull(value, "evidence");
  return isCurrentEvidence(value.evidence) && value.evidence.eid === value.field && value.evidence.value_display === value.display;
}
...
export function isCurrentSourceEntry(value: unknown): value is SourceEntry {
  if (!record(value) || !only(value, ["v", "index", "citation", "label", "snippet", "evidence", "evidence_omitted_count"]) || value.v !== 2 || !positiveInteger(value.index) || !isCurrentCitation(value.citation) || typeof value.label !== "string" || (!("snippet" in value) || nullableString(value.snippet)) === false || !Array.isArray(value.evidence) || !value.evidence.every(isCurrentEvidence) || !Number.isSafeInteger(value.evidence_omitted_count) || (value.evidence_omitted_count as number) < 0) return false;
