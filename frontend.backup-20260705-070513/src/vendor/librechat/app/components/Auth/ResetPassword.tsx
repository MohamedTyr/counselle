// Vendored from upstream client/src/components/Auth/ResetPassword.tsx @ 197a1dc4
// Subtractions: useResetPasswordMutation, hidden token/userId inputs + their
//   error spans + invalid-token setError path (no reset tokens in mock auth —
//   submit always shows the upstream success state).
// Rewire: submit → local isSuccess state + setHeaderText (upstream onSuccess).
...
function ResetPassword() {
  ...
  const { setHeaderText, startupConfig } = useOutletContext<TLoginLayoutContext>();
  ...
  const onSubmit = (_data: TResetPassword) => {
