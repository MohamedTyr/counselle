...
Skills are SKILL.md files (open standard: YAML frontmatter + Markdown body), living in `skills/`. Current skills include public response-mode workflows (`focused-answer`, `deep-research`, `guided-counselor`), public task workflows (`school-deep-dive`, `school-comparison`), and internal support workflows (`citation-and-recency`, `db-recipes`). Metadata loads at startup and bodies load through progressive disclosure. The non-advertised `dossier-assembly` alias canonicalizes to `school-deep-dive` only for parked-turn compatibility; it is not a public skill.
