...
5. **Reset emails** go through a thin `adapters/email.py` seam — provider and credentials in Settings; templates are data assets. **MVP2 implementation:** `console` (prints the reset link to logs) is the only built provider; `smtp`/`resend` are stubbed for later. The Settings field is `email_provider` (`Literal["console"]` today).
