...
5. **Reset emails** go through a thin `adapters/email.py` seam — provider and credentials in Settings; templates are data assets. `console` (prints the reset link to logs) is the only built provider; `smtp`/`resend` are stubbed for later. The Settings field is `email_provider` (`Literal["console"]` today).
...
**Additional decisions (verified against pinned dependencies — fastapi-users 15.0.5, httpx-oauth 0.17.0, pwdlib 0.3.0, pyjwt 2.13.0; 25/25 checks passed):**
...
- **Managed auth (Clerk / Auth0 / Supabase Auth)** — rejected: an external dependency, cost, and an egress of student PII for flows fastapi-users covers entirely; also weakens the self-contained one-container deploy. Reconsider if SSO/2FA/orgs ever matter.
...
- A JWT can't be revoked before expiry — bounded by the TTL (Settings knob); acceptable for the current threat model. The TTL is locked at 30 days with no refresh — revocation-before-expiry stays out of scope.
