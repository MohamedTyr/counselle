...
5. **Counselle-native components** (timeline, dossier stat block, comparison table, citation chips/popovers, sources footer, clarify widget, "not in our database" card) are built new, using only the cloned tokens, primitives, spacing, and motion timings — plus exactly two new semantic token pairs: `--official-*` (cool) and `--community-*` (warm).
