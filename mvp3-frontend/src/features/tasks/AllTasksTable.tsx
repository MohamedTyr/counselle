...
function handleAllTaskRowKeyDown(
            ...
            {sortedTasks.length === 0 ? (
              ...
              sortedTasks.map((task) => (
                <TableRow
                  ...
                  className={cn(
                    "cursor-pointer outline-none focus-visible:ring-3 focus-visible:ring-ring/45",
                    task.status === "done" && "opacity-70"
                  )}
                  key={task.id}
                  onClick={() => onOpenTask(task.id)}
                  onKeyDown={(event) =>
                    handleAllTaskRowKeyDown(event, task.id, onOpenTask)
                  }
                  tabIndex={0}
                >
                  <TableCell className="overflow-hidden py-3">
                    <AllTaskTitleCell onUpdateTask={onUpdateTask} task={task} />
                  </TableCell>
