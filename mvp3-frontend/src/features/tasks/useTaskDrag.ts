...
function createTaskDragImage({
  ...
  const previewElements = getTaskElementsForIds(surface, taskIds).slice(0, 3)
  ...
  previewElements.forEach((element, index) => {
    ...
    Object.assign(clone.style, {
      background: "color-mix(in oklch, var(--card), var(--ring) 14%)",
      borderColor: "transparent",
      boxShadow: "0 8px 18px oklch(0 0 0 / 26%)",
