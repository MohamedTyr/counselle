...
| `deploy/seed/cds_library_schema.sql` | The schema-DDL and grant-model source of record for `cds_library` — every base table, index, constraint, the five reader views, and the `cds_library_app` role's exact grant. `cds_library` is not created by this repo's yoyo migrations; this file is what makes a fresh environment provisionable. Referenced from `docs/DATABASE_GUIDE.md` §1. |
