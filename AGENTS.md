...
- **Current skill set:** public response modes `focused-answer`, `deep-research`, and `guided-counselor`; public task skills `school-deep-dive` and `school-comparison`; internal `citation-and-recency` and `db-recipes`. The hidden `dossier-assembly` alias exists only for parked-turn compatibility and is never advertised.
