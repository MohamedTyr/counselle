...
mocked browser QA, and focused live behavior eval cases are complete. After the
menu-polish review, the owner explicitly approved the result and requested the
feature be merged to `main`.
...
```text
Approved after the compact menu and spacing review; merge requested.
```
