...
## 20. Implementation status (2026-07-20)

**Shipped and verified:** the full public contract (`domain/mutation_receipts.py`, `app/workspace_mutation_receipts.py`), wiring into `StepDetail`/`EmissionRouter`/overflow (§6–§7.4), and all 29 tools across all 8 families emitting real typed receipts — backed by transaction-authoritative before/after diffs for tasks, schools, activities, honors, and essay metadata updates. The frontend contract (mirrored types, tolerant parser, shared glance formatter used by both the widget and copy/export, `MutationReceiptShell` with controlled disclosure, and a shared typed body renderer covering all 10 body kinds) is live and routes correctly through `ToolWidgets.tsx`'s priority rules (§11.1), including the marker-present/invalid → safe-unknown path. **All 8 families now have bespoke per-family anatomy widgets** (§9.1–9.8: `TaskMutationWidget`, `SchoolMutationWidget`, `EssayMutationWidget`, `EssayContentMutationWidget`, `ActivityMutationWidget`, `HonorMutationWidget`, `ProfileMutationWidget`, `MemoryMutationWidget`), sharing common formatters (`formatWordBudget`, `ChangeList`, `formatValue`) rather than duplicating logic. `/dev/tool-calls` carries real fixtures for every family. An automated accessibility pass (`jest-axe`, `mutation-receipt-a11y.test.tsx`) runs axe against every gallery fixture collapsed and expanded — 62 checks, zero violations. Full verification: backend `ruff`/`mypy` clean repo-wide, `pytest -m "not live_llm and not live_search"` and `pytest -m live_db` green (only 2 pre-existing, unrelated failures predating this work), frontend `tsc`/`eslint`/`vitest` green (692 tests).

**Deliberate, documented deviations from the full spec:**
- `essay content` word-count deltas and `reorder_activities`/`reorder_honors` intentionally omit authoritative old-rank/moved-index data where the underlying service doesn't yet capture it pre-transaction — the receipt correctly falls back to final-value-only per §7.3 rather than inferring movement.
- No *manual* live-browser/screen-reader/contrast verification (§16.3) has been performed — `jest-axe` catches ARIA/labeling/role defects in jsdom but cannot verify real computed color contrast or exercise a live screen reader (NVDA/VoiceOver). A real-browser Playwright pass was attempted as a partial substitute but the environment's Chrome install is a broken symlink to a missing cached build (root-owned path, not fixable without a system-level install this pass didn't attempt); that half of §16.3 needs a human-driven pass in a working browser environment.
- Service-layer atomicity hardening beyond what's described above (e.g. `create_essays`'/`create_honors`' all-or-nothing batch guarantees per §7.2) has not been independently re-audited in this pass.

This record graduates from `plans/` in this state — substantially complete and independently verified, with the above gaps tracked here rather than hidden. Follow-up work should update this section, not rewrite the narrative above it.
