...
| [`cds-pipeline/`](cds-pipeline/) | Moving the CDS extraction pipeline in-app (ADR 0036) — `DESIGN.md`, `METRICS-KEEP.md`/`METRICS-TRIAGE.md` (the 394-metric cut record), `plan/` (`PLAN.md`, `SHIP-PLAN.md`, `CUTOVER.md`, the six recon reports, the routing/flag-precision tuning writeups, and the two-round `cds-admin-polish-2.md` audit of the admin feature plus its `cds-admin-polish-2-live-schema.md` live-DB snapshot), and `tuning/` (the metric-cut prose reports plus the five kept harness scripts and sealed ground-truth corpus that reproduce it). See `plan/SHIP-PLAN.md` §4 for the full gated definition of done. | Ship gate (functional pipeline) and merge gate (mergeable branch) status recorded in `plan/SHIP-PLAN.md` §4; the `cds-admin-polish-2` fix batch also shipped (`AGENTS.md` Status); owner acceptance pending per `AGENTS.md` |
| [`essay-creation-simplification/`](essay-creation-simplification/) | Erasing the unused school essay-prompt catalog (three tables, four triggers, the mixed-source create dialog) in favor of the model a student actually has: a personal statement picked from the seven standard Common App prompts, and a supplement whose prompt is typed or left blank — `plan/implementation-plan.md` plus an appended implementation note recording what differed from the plan (migration renumbering, a stale runtime-guard target, `SchoolWorkspace.tsx`'s prior split, the dropped "650 words" figure, the `PromptMenu` add/edit form, and two SSE replay bugs found during the work). | Shipped |
