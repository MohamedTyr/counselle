...
| [`counseling-response-modes/`](counseling-response-modes/) | Skills-backed composer counseling modes — Focused Answer, Deep Research, and Guided Counselor as public response-mode SKILL.md workflows, exposed through one primary mode selector while preserving specialized `@` skills. | Implemented; deterministic suites and mocked-browser UX pass complete, live behavior eval/owner acceptance pending environment/human sign-off |
