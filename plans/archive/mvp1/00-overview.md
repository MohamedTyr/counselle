...
1. **Dispatch builders.** Split the phase file's work breakdown into its named slices. One agent per slice. **Model routing (updated 2026-06-10, token optimization): Sonnet for easy/prescriptive tasks AND most reviews; Opus/Fable only for genuinely hard tasks** (deep design judgment, the honesty core, gnarly debugging). When a phase file's spec is detailed and prescriptive, Sonnet builds it. Each agent's prompt = the phase file section + the exact file list it may touch + the relevant doc excerpts named in the phase file. Independent slices run in parallel; dependent ones sequentially as marked.
...
3. **Review loop.** Dispatch in parallel: `code-reviewer`, `python-reviewer`, plus `security-reviewer` when the phase touches SQL/network/user input (phases 2, 4, 5, 6) and `silent-failure-hunter` when it adds error paths (phases 2, 4, 5). Reviewers run on Sonnet in most cases (Fable/Opus only for the hardest review surfaces, e.g. the honesty core); fixers follow the same routing rule as builders (Sonnet for prescriptive fixes, Fable/Opus only for hard ones). Collect findings → dispatch fixer agents for every CRITICAL/HIGH (and cheap MEDIUMs) → re-run tests → re-review **changed files only**. Repeat until a clean pass (zero CRITICAL, zero HIGH). Max 4 loop cycles; if still failing, stop and escalate to the user with the stuck findings.
