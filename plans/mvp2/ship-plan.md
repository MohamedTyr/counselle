...
- **`app/state.py`:** `TurnState` gains `turn_records` — **an overwrite channel where every writer owns the full list** (no reducer; the four writers — node return, cancel write, G3 rewrite, parked-clarify write — each read-prior-append-write; one sentence that prevents the double-append class). Record fields per G2. **`parts[]` carries materialized prose** (each text part holds its `text`; the record is self-contained, plus `user_text`) — **revised at B1b review:** the original "offsets into `messages`" design garbled prose on reload (the offsets were built over the delta stream but the transcript reconstructed from `result.all_messages()` text — the two diverge on a pre-clarify-text resume; two HIGH findings). The record now duplicates the assistant prose (~1–2KB/turn, on the TTL/growth watch list) so the transcript read never reconstructs by slicing `messages`. **`messages_offset` stays in the record purely as B2's G3 history-rewrite anchor** (run_turn computes it authoritatively per path and threads it via `turn_ids`; the read no longer uses it). The self-contained record still requires the invariant: **every terminal path (complete, cancelled, error, tool-budget) leaves `messages` carrying exactly the prose that streamed** (for the provider history + G3 slice). The `UsageLimitExceeded` path (today loses streamed prose — a live MVP1 bug under G2's lens) and the node-error path get the partial-`ModelResponse` + record treatment here; cancel gets it in B2; **the empty-partial rule: no prose streamed → skip the `ModelResponse` append entirely** (an empty-content response corrupts the provider history). Clarify: parked record written via spike-1's proven mechanism; **resume replaces it, same `message_id`** (G4).
