...
### 1.7 `step` — NEW event (B1a)

FE already declares the full shape in protocol.ts (`StepData`/`StepKind`/`StepTier`/`StepDetail`) — **backend B1a must serve exactly these names** (the new `domain/events.py` types are the sanctioned §26 carve-out; mapper in `app/steps.py` + `config/assets/step_labels.yaml`):
...
### 1.8 `thinking` — NEW event (B1a)

`data = { text: string }` — FE `ThinkingData` already declared; backend serves exactly that. Consumer: reducer `thinking[]` + interleaved `timeline`.
...
| `step_record` | `{ steps: StepData[], thinking: string[], receipt: string }` | optional (absent pre-MVP2) | declared (`StepRecord`) | no |
...
There is **no backend receipt today** (the harness renders no receipt; `_extract_transcript` is prose-only; architecture.md §27.1 specs only "the derived one-line receipt"). The only concrete format in the codebase is the FE's `deriveReceipt` (`turn-reducer.ts`) — and per the consumer-contract rule, **that format is pinned as the contract** (B1b implements it server-side so persisted receipts == live-derived receipts):
...
No edits needed for: `StepData`/`StepKind`/`StepTier`/`StepDetail`/`ThinkingData` (already declared, backend conforms to them), `DoneStatus` (already has `cancelled`), `status`/`error`/`message_id`/`ts`/`parts`/`step_record`/`sources`/`usage` on entries (already declared and conformant).

Backend mirror edits (B1a/B1b/B2, the checklist): `EventType` gains `"step" | "thinking"`; `StepData`/`ThinkingData` models + `ev_step`/`ev_thinking`; `MetaData` + `ev_meta` gain the two ids; `DoneData.status` gains `"cancelled"`; `MessageBody` gains `replace_message_id: str | None` (G3); the transcript read serializes §2.
...
- `src/api/transport.ts` — `SendMessageBody.source_config: SourceConfigWire`; add `replace_message_id?: string` (G3); `VITE_TRANSPORT` switch built for real.
- `src/api/source-config.ts` (NEW) — the §4 mapper, both directions.
- `src/api/turn-reducer.ts` — expose `meta`'s ids; carry the clarify **answer** in view-state; `transcriptEntryToEvents` reads `clarify.spec` (+ threads `answer` outside the event replay); default `status` for pre-MVP2 entries unchanged.
